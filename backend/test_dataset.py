...
class TestDataset(data.Dataset):
    def __init__(self, database_folder, queries_folder, positive_dist_threshold=25, image_size=None, use_labels=True):
        ...
        positive_dist_threshold : int, distance in meters for a prediction to
            be considered a positive (not used anymore, kept for compatibility).
        image_size : tuple or int, size to resize images to.
        use_labels : bool, whether to use labels (not used anymore, kept for compatibility).
        """
        super().__init__()
